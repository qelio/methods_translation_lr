...
def preprocess_code(filename: str) -> Optional[str]:
    try:
    ...
    if code.count('/*') != code.count('*/'):
        print("PREPROCESSOR_ERROR: Незакрытый многострочный комментарий")
        return None
    ...
    print("\nЛР1. ПРЕПРОЦЕССИНГ")
    print("-" * 60)
    print(f"Очищенный код сохранён в файл: {cleaned_filename}")
...
# ============================================================
# ЛР4. СЕМАНТИЧЕСКИЙ АНАЛИЗАТОР И ТРИАДЫ
# ============================================================
...
class SemanticAnalyzer:
    ...
    def print_triads(self):
        print("\nПромежуточное представление в виде триад")
        print("-" * 60)
...
# ============================================================
# ЕДИНЫЙ ЗАПУСК КОМПИЛЯТОРА ЛР1–ЛР4
# ============================================================
...
def main():
    ...
    print(f"\nВходной файл: {filename}")

    cleaned_code = preprocess_code(filename)

    if cleaned_code is None:
        print("\nКомпиляция остановлена на этапе препроцессинга.")
        return
    ...
    if lexer.errors:
        print("\nКомпиляция остановлена на этапе лексического анализа.")
        return
    ...
    if parser.errors:
        print("\nКомпиляция остановлена на этапе синтаксического анализа.")
        return
    ...
    if semantic.errors:
        print("\nКомпиляция завершена с семантическими ошибками.")
        return

    semantic.print_triads()

    print("\nПолный цикл обработки программы завершён успешно.")
